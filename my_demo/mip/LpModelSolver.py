...
# import os
# # 禁用Gurobi自动检测
# os.environ["PULP_IGNORE_SOLVERS"] = "GUROBI,GUROBI_CMD" # PuLP >= 2.6
# os.environ["GRB_ISV_NAME"] = ""               # 阻止Gurobi初始化
# # 检查已安装求解器
# print("可用求解器:", pulp.listSolvers(onlyAvailable=True))
# # 验证Gurobi不在列表中
# assert 'GUROBI' not in [s.upper() for s in pulp.listSolvers()]
import pulp
from collections import defaultdict
import os


class LpModelSolver(ModelSolver):
    ...
    def init_model(self):
        ...
        for i, j in self.data_holder.all_arcs:
            self.x[i].update({j: pulp.LpVariable(f"x_[{i},{j}]", cat=pulp.LpContinuous,lowBound=0, upBound=1)})

            self.y[i].update({j: pulp.LpVariable(f"y_[{i},{j}]", cat=pulp.LpContinuous,lowBound=0, upBound=1)})
